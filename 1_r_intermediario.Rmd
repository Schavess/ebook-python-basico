...
O pacote `data.table` é uma extensão do data.frame que combina alta eficiência com uma sintaxe poderosa para manipulação de dados. Ele é especialmente projetado para trabalhar com grandes conjuntos de dados, proporcionando melhor desempenho e flexibilidade em comparação com estruturas tradicionais como o próprio `data.frame`.
...
```
<br>
Adicionando Duas Novas Colunas com `:=`

Podemos adicionar uma nova coluna chamada valor_2 que seja o dobro do valor existente na coluna valor:
```{r}
DT[, valor_3 := valor * 3][, valor_4 := valor * 4]
...
library(reshape2)
contagem <- table(dados$Especie, dados$Parcela)
